# club_comparison.R -----------------------------------------------------------
# Ahanor against the comparison club's centre-back room - the "gap he fills"
# chart. Each metric is a row on a common percentile scale; the club's CBs
# are dots with a band spanning their range, Ahanor is the accent dot.
# Rows where his dot sits beyond the band are the gap.
#
# Percentiles are computed against ALL Big-5 defenders (any age, 600+ league
# minutes, current season) so his U23 status doesn't distort the scale -
# the club's CBs are mostly older.
#
# Reads  data/raw/pool_defenders_all.csv   (04_build_pool.py)
#        data/raw/club_cbs.csv             (05_scrape_club_cbs.py)
...
TITLE_CLUB <- "Measured for the Chelsea Back Line"

pool <- readr::read_csv("data/raw/pool_defenders_all.csv",
                        show_col_types = FALSE) |>
  filter(season == CURRENT_SEASON)
club <- readr::read_csv("data/raw/club_cbs.csv", show_col_types = FALSE) |>
  mutate(is_cb = toupper(as.character(is_cb)) %in% "TRUE")

club_name <- club$team_name[1]
cbs <- club |> filter(is_cb)
if (nrow(cbs) < 2) {
  message("fewer than 2 tagged CBs - using all ", club_name, " defenders")
  cbs <- club
}
...
if (nrow(cbs_in_pool) == 0) stop("no ", club_name, " CBs have pool rows")
...
targets <- bind_rows(
  cbs_in_pool |> mutate(role = paste0(club_name, " centre-backs")),
  subject |> mutate(role = "Honest Ahanor")
)
...
club_role <- paste0(club_name, " centre-backs")
band <- plot_df |>
...
caption <- chart_footer(paste0(
  "Percentiles vs all Big-5 defenders (any age), min. 600 league minutes, ",
